      subroutine umat_fcc_dsl_porous(cm,eps,sig,epsp,hsv,dt1,capa,etype,tt,
...
c      double precision ec11,ec12,ec44
      double precision cmat(6,6),cmat_cry(6,6)
...
!============================================================
! Declaration of damage model variables
!------------------------------------------------------------
! Note:
!------------------------------------------------------------
      integer dmg_type
      double precision vf_0,fn,peeqn,sn
      double precision sm_d
      double precision vf,mf,vf_n,vf_g
      double precision vfr,vfr_n,vfr_g
      double precision cdmat(6,6),cdgeomat(6,6)
      double precision esbsmat(6,6),geomat(6,6)
...
c     Strain variables
      ...
      double precision eeqr,peeqr
      double precision eeq_n1,peeq_n1
...
c     diagonal part of deformation gradient elastic
            ...
            hsv(133)=1.
            hsv(137)=1.
c     void volume fraction at current step
            hsv(138)=vf_0
      else
!============================================================
! Calculation begins
...
! Note:
!------------------------------------------------------------  
c     calculate the resolved shear stress
...
c     calculate the effective crss
      call edge_dsl_line_vec(m11,s11,num_ss,l11)
      call tm_porous_sm_sphere(sm,vf,pr,sm_d)
      call cal_crss_by_dsl_Lee(ga,sm_d,bv,rho,m11,s11,l11,
     1     num_ss,g_crss)
c     calculate the slip and update the cumulative slip
...
c     Damage evolution
      call porous_evl_gtn_nucleation(peeq_n1,peeqr,fn,peeqn,sn,vfr_n)
      call porous_evl_gtn_growth(vf,Dp,vfr_g)
      vf_n=vf_n+vfr_n*dt1
      vf_g=vf_g+vfr_g*dt1
      vfr=0.0
      vfr=vfr_n+vfr_g
...
c     Fourth-order elastic tensor at crystal coordinate
      call calElasIsoTensor(ym,pr,cmat_cry)
c     tm scheme - damage
      mf=1.0-vf
...
c     Transform elastic tensor to material coordinate
      ...
      call tranElasTensorLocal2Global(cdgeomat,RL,cmat)
...
! Note:
!------------------------------------------------------------ 
      end subroutine umat_fcc_dsl_porous